...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
     ...
     */
    @Autowired
    private ItFxqAuthSuccessHandler itFxqAuthSuccessHandler;
    /**
     * 依赖注入自定义的登录失败处理器
     */
    @Autowired
    private ItFxqAuthFailureHandler itFxqAuthFailureHandler;
    /**
     * 依赖注入自定义的注销成功的处理器
     */
    @Autowired
    private ItFxqLogoutSuccessHandler itFxqLogoutSuccessHandler;
    /**
     * 注册没有权限的处理器
     */
    @Autowired
...
//    }
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.authorizeRequests()
                ...
                //配置登录失败的自定义处理类
                .failureHandler(itFxqAuthFailureHandler)
                .and()
                //loginProcessingUrl用于指定前后端分离的时候调用后台注销接口的名称
                .logout().logoutUrl("/logout")
                .logoutSuccessHandler(itFxqLogoutSuccessHandler)
                .and()
    ...
    private UserAuthenticationFilter UserAuthenticationFilterBean() throws Exception {
        ...
        userAuthenticationFilter.setAuthenticationSuccessHandler(itFxqAuthSuccessHandler);
